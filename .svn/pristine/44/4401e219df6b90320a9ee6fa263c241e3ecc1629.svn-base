...
public class DiscListAdapter extends BaseAdapter {
	...
	Context context;
	public LinkedList<DiscListBean> list;

	public DiscListAdapter(Context context) {
		this.context = context;
		list = new LinkedList<DiscListBean>();
	}

	public void setArrayList(LinkedList<DiscListBean> list) {
		this.list = list;
	...
	@Override
	public DiscListBean getItem(int position) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final DiscListBean beanLeft = list.get(position * 2);
		ImageView imagLeft = ViewHolder.get(convertView,
				R.id.item_disc_img_left);
		BitmapAsset.displayRec(context, imagLeft, beanLeft.getPhoto());

		TextView titleLeft = ViewHolder.get(convertView,
				R.id.item_disc_tv_title_left);
		titleLeft.setText(beanLeft.getContent());

		Button viewCountLeft = ViewHolder.get(convertView,
				R.id.item_disc_btn_msg_left);
		viewCountLeft.setText("" + beanLeft.getLeaveWordCount());

		TextView timeLeft = ViewHolder.get(convertView,
				R.id.item_disc_tv_time_left);
		timeLeft.setText(beanLeft.getReleaseTime());
		TextView catogeryLeft = ViewHolder.get(convertView,
				R.id.item_disc_tv_catogery_left);
		...
		layoutLeft.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				intent.putExtra(DiscDetailActivity.KEY_DISCLIST, beanLeft);
		...
		final DiscListBean beanRight = list.get(position * 2 + 1);
		ImageView imagRight = ViewHolder.get(convertView,
				R.id.item_disc_img_right);
		TextView titleRight = ViewHolder.get(convertView,
				R.id.item_disc_tv_title_right);
		Button viewCountRight = ViewHolder.get(convertView,
				R.id.item_disc_btn_msg_right);
		TextView timeRight = ViewHolder.get(convertView,
				R.id.item_disc_tv_time_right);
		...
		TextView catogeryRight = ViewHolder.get(convertView,
				R.id.item_disc_tv_catogery_right);
		...
		layoutRight.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				Intent intent = new Intent(context, DiscDetailActivity.class);
				intent.putExtra(DiscDetailActivity.KEY_DISCLIST, beanRight);
